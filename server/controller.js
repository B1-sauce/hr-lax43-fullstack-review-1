...
const controller = {
  students: {
    getStudents: function (req, res) {
      ...
      const queryStr = 'SELECT * FROM students INNER JOIN images WHERE students.id = images.id';
    ...
    deleteStudent: function(req, res){
      const id = req.params.id;
      const queryStr = `DELETE students, images FROM students INNER JOIN images WHERE students.id=images.id AND students.id=${id}`;
      db.query(queryStr, (err, result) => {
        if (err) {
          res.status(400).send(err)
        } else {
